...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `${c.name}${c.city ? ` in ${c.city}` : ''} – Minijob-Arbeitgeber | Minijobgenie`;
  const description = c.shortDescription || c.description?.substring(0, 155) || `${c.name} – Minijob-Arbeitgeber auf Minijobgenie. Offene Stellen, Bewertungen und Videos.`;
...
export default async function FirmenDetailPage({ params }: Props) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Firmen', item: 'https://minijobgenie.de/firmen' },
    ...
    <main className="min-h-screen bg-gray-50">
      ...
      <section className="relative pt-20 pb-8">
        {c.coverImageUrl ? (
          ...
          <div className="absolute inset-0 h-48 bg-gradient-to-br from-emerald-500 via-emerald-600 to-green-600">
            <div className="absolute inset-0 bg-gradient-to-t from-gray-50 to-transparent" />
          </div>
        )}
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 sm:p-8">
            <div className="flex flex-col sm:flex-row items-start gap-5">
              {c.logoUrl ? (
                ...
                <div className="w-20 h-20 rounded-xl bg-gradient-to-br from-emerald-100 to-green-100 flex items-center justify-center shadow-sm">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {c.verified && <CheckCircle2 className="h-6 w-6 text-emerald-500" />}
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3 text-sm text-gray-500 mb-3">
                  ...
                  {c.industry && (
                    <span className="inline-block bg-emerald-50 text-emerald-700 px-2.5 py-0.5 rounded-full text-xs font-medium">
                      {c.industry}
                    </span>
                  )}
                  {c.foundedYear && (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6 pt-6 border-t border-gray-100">
              <div className="text-center">
                <div className="text-2xl font-bold text-emerald-600">{c.jobCount}</div>
                <div className="text-xs text-gray-500">Offene Stellen</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-emerald-600">{c.videoCount}</div>
                <div className="text-xs text-gray-500">Videos</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-emerald-600">{c.followerCount}</div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        <nav className="text-sm text-gray-500 mb-6" aria-label="Breadcrumb">
          <ol className="flex flex-wrap items-center gap-1">
            ...
            <li>/</li>
            <li><Link href="/firmen" className="hover:text-emerald-600">Firmen</Link></li>
            <li>/</li>
                ...
                <div className="space-y-3">
                  {jobs.map((job) => (
                    <Link
                      ...
                      className="block group p-4 rounded-lg border border-gray-100 hover:border-emerald-200 hover:bg-emerald-50/30 transition-all"
                    >
                  ...
                  {videos.map((video) => (
                    <div key={video.id} className="group relative rounded-lg overflow-hidden bg-gray-900 aspect-video">
                      {video.thumbnailUrl ? (
                        ...
                        <div className="w-full h-full bg-gradient-to-br from-emerald-600/20 to-green-600/20" />
                      )}
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-5">
                ...
                {c.trainingSince && (
                  <div className="text-sm text-gray-600 mb-2">
                    Aktiv seit <span className="font-semibold text-gray-900">{c.trainingSince}</span>
                  </div>
                )}
                {c.totalApprentices && (
                  <div className="text-sm text-gray-600">
                    <span className="font-semibold text-gray-900">{c.totalApprentices}</span> Mitarbeiter im Minijob
                  </div>
                )}
              </div>
            )}
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              ...
              <div className="space-y-2">
                {c.jobCount > 0 && (
                  <a
                    href="#offene-stellen"
                    className="flex items-center gap-2 w-full px-4 py-2.5 rounded-lg bg-emerald-600 text-white font-medium text-sm hover:bg-emerald-700 transition-colors"
                  >
