...
export const metadata: Metadata = {
  title: 'Arbeitgeber für Minijobs entdecken | Minijobgenie',
  description: 'Finde deinen Minijob-Arbeitgeber: Firmenprofile, Bewertungen, offene 450€-Jobs und Videos. Entdecke Top-Arbeitgeber für Minijobs in deiner Region.',
  openGraph: {
    title: 'Arbeitgeber für Minijobs entdecken | Minijobgenie',
    description: 'Finde deinen Minijob-Arbeitgeber: Firmenprofile, Bewertungen, offene 450€-Jobs und Videos.',
  },
};
...
export default async function FirmenPage({ searchParams }: Props) {
  ...
  const [data, stats] = await Promise.all([
    ...
    fetchApi<CompanyStatsResponse>('/public/companies/stats?portal_id=4'),
  ]);
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-emerald-50/50 to-white">
      ...
      <section className="relative pt-24 pb-12 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-emerald-500 via-emerald-600 to-green-600" />
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-white mb-4">
            Arbeitgeber für Minijobs entdecken
          </h1>
          <p className="text-lg text-white/80 max-w-2xl mx-auto mb-8">
            Finde den passenden Arbeitgeber für deinen Minijob. Vergleiche Firmen, lies Bewertungen und entdecke offene 450€-Jobs.
          </p>
          ...
          <form className="max-w-2xl mx-auto" action="/firmen">
            <div className="flex bg-white rounded-xl shadow-xl overflow-hidden">
              ...
              <button
                type="submit"
                className="px-6 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold transition-colors"
              >
                Suchen
              </button>
            </div>
          ...
          <aside className="hidden lg:block">
            {stats && (
              <div className="bg-white rounded-xl border border-gray-200 p-5 mb-6">
                ...
                <div className="text-3xl font-extrabold text-emerald-600 mb-1">{stats.totalCompanies}</div>
                <div className="text-sm text-gray-500 mb-4">Minijob-Arbeitgeber</div>
              </div>
            )}
          ...
          <div className="lg:col-span-3">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
              <div className="flex items-center gap-2 text-sm text-gray-600">
                <span className="font-semibold text-gray-900">{total}</span> Arbeitgeber gefunden
                {searchParams.q && (
                  <span className="inline-flex items-center gap-1 bg-emerald-50 text-emerald-700 px-2.5 py-1 rounded-full text-xs font-medium">
                    &quot;{searchParams.q}&quot;
                    <Link href="/firmen" className="hover:text-emerald-900">&times;</Link>
                  </span>
                )}
                {searchParams.city && (
                  <span className="inline-flex items-center gap-1 bg-emerald-50 text-emerald-700 px-2.5 py-1 rounded-full text-xs font-medium">
                    {searchParams.city}
                    <Link href={`/firmen${searchParams.q ? `?q=${searchParams.q}` : ''}`} className="hover:text-emerald-900">&times;</Link>
                  </span>
                )}
              </div>
            ...
            {companies.length > 0 ? (
              <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4">
                {companies.map((company) => (
                  <Link
                    ...
                    className="group bg-white rounded-xl border border-gray-200 hover:border-emerald-300 hover:shadow-lg transition-all p-5"
                  >
                    ...
                    {company.industry && (
                      <span className="inline-block text-xs font-medium bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-full mb-2">
                        {company.industry}
                      </span>
                    )}
              ...
              <div className="text-center py-16 bg-white rounded-xl border border-gray-200">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mb-2">Keine Arbeitgeber gefunden</h3>
                <p className="text-gray-500 mb-4">Versuche es mit anderen Suchbegriffen oder Filtern.</p>
                <Link href="/firmen" className="text-emerald-600 hover:text-emerald-700 font-medium">
                  Alle Arbeitgeber anzeigen &rarr;
                </Link>
              </div>
            )}
      ...
      {/* CTA */}
      <section className="bg-gradient-to-r from-emerald-500 to-green-600 py-12">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <h2 className="text-2xl font-bold text-white mb-3">Du bist Arbeitgeber?</h2>
          <p className="text-white/80 mb-6">Erstelle dein Firmenprofil und finde die besten Aushilfen über unsere Plattform.</p>
          <a
