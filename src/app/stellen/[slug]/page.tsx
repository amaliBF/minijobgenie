...
export async function generateMetadata({
  ...
  if (city) {
    return {
      title: `Minijobs in ${city.name} 2026 – Aktuelle 538€-Jobs`,
      description: `Finde deinen Minijob in ${city.name}! Aktuelle 538€-Jobs und Nebenjobs von Arbeitgebern aus ${city.name} und Umgebung. Mit Video-Einblicken.`,
      alternates: { canonical: `/stellen/${params.slug}` },
      openGraph: {
        title: `Minijobs in ${city.name} | Minijobgenie`,
        description: `Aktuelle Minijobs in ${city.name} und Umgebung.`,
        url: `https://minijobgenie.de/stellen/${params.slug}`,
      },
    };
  }
  ...
  return {
    title: job.metaTitle || `${job.title} bei ${company.name}${cityText ? ` | ${cityText}` : ''} – Minijob`,
    description:
      job.metaDescription ||
      `${job.title} bei ${company.name}${cityText ? ` in ${cityText}` : ''}. Jetzt auf Minijobgenie bewerben!`,
    alternates: { canonical: `/stellen/${params.slug}` },
    openGraph: {
      title: `${job.title} bei ${company.name}`,
      description: `${job.title}${cityText ? ` in ${cityText}` : ''}. Jetzt bewerben!`,
      url: `https://minijobgenie.de/stellen/${params.slug}`,
    },
...
function CityPage({
  ...
  const jsonLd = {
    ...
    '@type': 'CollectionPage',
    name: `Minijobs in ${city.name}`,
    description: `${total} aktuelle Minijobs in ${city.name}`,
    url: `https://minijobgenie.de/stellen/${slug}`,
    breadcrumb: {
      '@type': 'BreadcrumbList',
      itemListElement: [
        { '@type': 'ListItem', position: 1, name: 'Stellen', item: 'https://minijobgenie.de/stellen' },
        { '@type': 'ListItem', position: 2, name: city.name, item: `https://minijobgenie.de/stellen/${slug}` },
      ],
    },
  };
  ...
  return (
    <div className="min-h-screen bg-[#F0FDF4]">
      ...
      <section className="pt-28 pb-8 bg-gradient-to-b from-emerald-50/60 to-[#F0FDF4]">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Breadcrumb */}
          <nav className="flex items-center gap-1.5 text-sm text-gray-500 mb-6">
            <Link href="/stellen" className="hover:text-emerald-600">Stellen</Link>
          ...
          <p className="mt-3 text-gray-600">
            {total > 0
              ? `${total} aktuelle Minijobs in ${city.name} und Umgebung`
              : `Noch keine Minijobs in ${city.name} – bald kommen neue!`}
          </p>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        {jobs.length > 0 ? (
          ...
          <div className="text-center py-16 bg-white rounded-xl border border-gray-200 mb-12">
            ...
            <p className="text-gray-600 font-medium">
              Noch keine Minijobs in {city.name}
            </p>
            <p className="text-sm text-gray-500 mt-1">
              Schau in der App vorbei – dort findest du Arbeitgeber per Video!
            </p>
          </div>
        )}
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 mb-12">
          <h2 className="font-bold text-gray-900 mb-2">
            Minijobs in {city.name} – {city.bundesland}
          </h2>
          <p className="text-sm text-gray-600 mb-4">
            Weitere Informationen über Minijobs und Nebenjobs in {city.name} findest du auf unserer Städte-Seite.
          </p>
          <Link
            href={`/minijobs/${slug}`}
            className="text-sm text-emerald-600 hover:text-emerald-700 font-medium inline-flex items-center gap-1"
          >
            Mehr über Minijobs in {city.name}
...
function JobDetailPage({
  ...
  const requirementsList = Array.isArray(job.anforderungen) && job.anforderungen.length > 0
    ...
    ? job.anforderungen.split(/[,;]/).map((r) => r.trim()).filter(Boolean)
        ...
        ? [
            {
              ...
              position: 2,
              name: cityText,
              item: `https://minijobgenie.de/stellen/${cityText.toLowerCase().replace(/[äÄ]/g, 'ae').replace(/[öÖ]/g, 'oe').replace(/[üÜ]/g, 'ue').replace(/ß/g, 'ss').replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '')}`,
            },
          ]
        : []),
      {
        ...
        name: job.title,
        item: `https://minijobgenie.de/stellen/${slug}`,
      },
    ],
  };
      ...
      <div className="pt-24 pb-16">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-6 sm:p-8 mb-6">
            <div className="flex items-start gap-5">
              {company.logoUrl ? (
                ...
                <div className="flex-shrink-0 w-16 h-16 rounded-xl bg-gradient-to-br from-emerald-50 to-green-50 flex items-center justify-center border border-emerald-100">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6 pt-6 border-t border-gray-100">
              ...
              {job.arbeitszeit && (
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-gray-700">{job.arbeitszeit}</span>
                </div>
              )}
            </div>
          </div>
              ...
              {videos.length > 0 && (
                <div className="bg-white rounded-2xl border border-gray-200 p-6">
                  <h2 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
                    ...
                    Video{videos.length > 1 ? 's' : ''} vom Arbeitgeber
                  </h2>
                  ...
                  <div className="space-y-2">
                    {job.salaryYear1 && (
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-600">Monatliche Vergütung</span>
                        <span className="font-medium text-gray-900">{job.salaryYear1.toLocaleString('de-DE')}€ / Monat</span>
                      </div>
                    )}
                  </div>
                </div>
          ...
          {similarJobs.length > 0 && (
            <div className="mt-12">
              <h2 className="text-xl font-bold text-gray-900 mb-6">Ähnliche Minijobs</h2>
...
export default async function StellenSlugPage({
  ...
  if (city) {
    const data = await fetchApi<ByCityResponse>(
      `/public/jobs/by-city/${params.slug}?portal_id=4`,
    );
